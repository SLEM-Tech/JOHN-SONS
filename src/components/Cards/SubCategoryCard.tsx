...
const SubCategoryCard = ({ id, image, name }: SubCategoryCardProps) => {
  ...
  return (
    <Link
      ...
      className={`flex flex-col gap-2 items-center group w-fit cursor-pointer rounded-sm border-[2px] border-transparent hover:border-primary/50 transition shrink-0 ${
        data === id ? "border-[2px]" : ""
      }`}>
